...
export function createSovereignTools(
  ...
  return {
    // ========================================================================
    // Marketplace integrations (installed providers + server env keys)
    // ========================================================================

    invokeMarketplaceIntegration: tool({
      description:
        "Run a marketplace integration tool after the user installed it in Marketplace. Examples: resend/email:send, openai/llm:embed, redis/cache:get, supabase/db:query_data. Server must have the provider API keys set. Successful runs are recorded in integration usage history (agent_id: sovereign).",
      ...
      execute: async (input) => {
        try {
          ...
          return await runInstalledMarketplaceIntegrationWithUsageLog({
            userId,
            ...
            agentId: "sovereign",
            logLabel: "[sovereign] integration usage log",
            usageSupabase: supabaseServiceRole(),
          })
        } catch (err: unknown) {
          return { ok: false, error: err instanceof Error ? err.message : "Marketplace execution failed." }
        }
      },
    }),
    ...
    // ═══════════════════════════════════════════════════════════════════════
    sovereignCanvas: tool({ description: "Create or update a strategy canvas snapshot", inputSchema: z.object({ canvasName: z.string(), strategyJson: z.string().optional(), judgmentDensityScore: z.number().optional() }), execute: async (input) => { try { const { data, error } = await client.from("sovereign_canvas").insert({ owner_user_id: userId, canvas_name: input.canvasName, strategy_json: input.strategyJson ? JSON.parse(input.strategyJson) : {}, judgment_density_score: input.judgmentDensityScore ?? 0 }).select().single(); if (error) throw error; return { ok: true, canvas: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    overnightRetraining: tool({ description: "Trigger overnight model retraining based on surprise deltas", inputSchema: z.object({ canvasId: z.string(), retrainingStatus: z.enum(["queued", "running", "complete", "failed"]).default("queued") }), execute: async (input) => { try { const { data, error } = await client.from("sovereign_canvas").update({ overnight_retraining_status: input.retrainingStatus, revision_count: 1 }).eq("id", input.canvasId).eq("owner_user_id", userId).select().single(); if (error) throw error; return { ok: true, canvas: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    measureJudgmentDensity: tool({ description: "Compute judgment quality density per domain", inputSchema: z.object({ domain: z.string(), decisionsPerHour: z.number(), qualityScore: z.number().min(0).max(100) }), execute: async (input) => { try { const densityRank = Math.round(input.qualityScore * Math.log2(Math.max(1, input.decisionsPerHour))); const { data, error } = await client.from("judgment_density_log").insert({ owner_user_id: userId, domain: input.domain, decisions_per_hour: input.decisionsPerHour, quality_score: input.qualityScore, density_rank: densityRank }).select().single(); if (error) throw error; return { ok: true, judgment: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    licenseTribalIp: tool({ description: "Create a P2P IP licensing agreement for tribal workflow assets", inputSchema: z.object({ tribeId: z.string().optional(), assetName: z.string(), licenseType: z.enum(["standard", "exclusive", "open"]).default("standard"), royaltyPct: z.number().min(0).max(100).default(10) }), execute: async (input) => { try { const { data, error } = await client.from("tribal_ip_licenses").insert({ owner_user_id: userId, tribe_id: input.tribeId, asset_name: input.assetName, license_type: input.licenseType, royalty_pct: input.royaltyPct }).select().single(); if (error) throw error; return { ok: true, license: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    auditTargetingIntegrity: tool({ description: "Verify targeting integrity score for IP assets", inputSchema: z.object({ licenseId: z.string(), integrityScore: z.number().min(0).max(100) }), execute: async (input) => { try { const { data, error } = await client.from("tribal_ip_licenses").update({ targeting_integrity_score: input.integrityScore }).eq("id", input.licenseId).eq("owner_user_id", userId).select().single(); if (error) throw error; return { ok: true, license: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),

    // ═══════════════════════════════════════════════════════════════════════
    // Protector Sovereign (#533-537)
    // ═══════════════════════════════════════════════════════════════════════
    runBiometricVetting: tool({ description: "Execute real-time biometric identity vetting at the edge", inputSchema: z.object({ subjectName: z.string(), vettingType: z.enum(["standard", "enhanced", "rapid"]).default("standard"), confidencePct: z.number().min(0).max(100) }), execute: async (input) => { try { const st = input.confidencePct >= 95 ? "verified" : input.confidencePct >= 70 ? "review" : "flagged"; const { data, error } = await client.from("biometric_vetting_results").insert({ owner_user_id: userId, subject_name: input.subjectName, vetting_type: input.vettingType, confidence_pct: input.confidencePct, status: st }).select().single(); if (error) throw error; return { ok: true, vetting: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    triageSignals: tool({ description: "Prioritize incoming threat signals via zero-knowledge query protocol", inputSchema: z.object({ vettingId: z.string(), priority: z.enum(["critical", "high", "normal", "low"]).default("normal") }), execute: async (input) => { try { const { data, error } = await client.from("biometric_vetting_results").update({ signal_triage_priority: input.priority }).eq("id", input.vettingId).eq("owner_user_id", userId).select().single(); if (error) throw error; return { ok: true, vetting: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    syncIncident: tool({ description: "Sync incident data across tribal systems via A2A protocol", inputSchema: z.object({ incidentId: z.string().optional(), sourceSystem: z.string(), targetSystem: z.string() }), execute: async (input) => { try { const { data, error } = await client.from("incident_sync_log").insert({ owner_user_id: userId, incident_id: input.incidentId, source_system: input.sourceSystem, target_system: input.targetSystem, sync_status: "synced" }).select().single(); if (error) throw error; return { ok: true, sync: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    neutralizeRepression: tool({ description: "Detect and neutralize transnational repression vectors", inputSchema: z.object({ syncId: z.string() }), execute: async (input) => { try { const { data, error } = await client.from("incident_sync_log").update({ repression_neutralized: true }).eq("id", input.syncId).eq("owner_user_id", userId).select().single(); if (error) throw error; return { ok: true, sync: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    runDefenseAudit: tool({ description: "Execute defense posture audit — analyze threat level and vulnerabilities", inputSchema: z.object({ auditScope: z.string(), threatLevel: z.enum(["critical", "high", "medium", "low"]).default("low"), vulnerabilitiesFound: z.number().int().default(0), remediationPlan: z.string().optional() }), execute: async (input) => { try { const score = Math.max(0, 100 - input.vulnerabilitiesFound * 5 - (input.threatLevel === "critical" ? 40 : input.threatLevel === "high" ? 25 : input.threatLevel === "medium" ? 10 : 0)); const { data, error } = await client.from("defense_audit_results").insert({ owner_user_id: userId, audit_scope: input.auditScope, threat_level: input.threatLevel, vulnerabilities_found: input.vulnerabilitiesFound, remediation_plan: input.remediationPlan, score }).select().single(); if (error) throw error; return { ok: true, audit: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),

    // ═══════════════════════════════════════════════════════════════════════
    // Global Human (#538-542)
    // ═══════════════════════════════════════════════════════════════════════
    initializeSovereignEnterprise: tool({ description: "Provision a full-stack sovereign enterprise for $0 capital", inputSchema: z.object({ enterpriseName: z.string(), enterpriseType: z.enum(["startup", "cooperative", "artisan", "nonprofit"]).default("startup"), artisanshipDomain: z.string().optional() }), execute: async (input) => { try { const { data, error } = await client.from("sovereign_enterprises").insert({ owner_user_id: userId, enterprise_name: input.enterpriseName, enterprise_type: input.enterpriseType, artisanship_domain: input.artisanshipDomain, capital_required_usd: 0 }).select().single(); if (error) throw error; return { ok: true, enterprise: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    detectLatentArtisanship: tool({ description: "Identify passion domains blocked by technical or physical hurdles", inputSchema: z.object({ skillDomain: z.string(), blockers: z.array(z.string()).default([]), recommendations: z.array(z.string()).default([]) }), execute: async (input) => { try { const latentScore = Math.min(100, 50 + input.recommendations.length * 10 - input.blockers.length * 5); const { data, error } = await client.from("latent_artisanship_results").insert({ owner_user_id: userId, skill_domain: input.skillDomain, latent_score: latentScore, blockers: input.blockers, recommendations: input.recommendations }).select().single(); if (error) throw error; return { ok: true, artisanship: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    auditAgenticReasoning: tool({ description: "Protect against slow hallucinations in high-stakes judgment", inputSchema: z.object({ artisanshipId: z.string(), passed: z.boolean() }), execute: async (input) => { try { const { data, error } = await client.from("latent_artisanship_results").update({ reasoning_audit_passed: input.passed }).eq("id", input.artisanshipId).eq("owner_user_id", userId).select().single(); if (error) throw error; return { ok: true, artisanship: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    provisionResonanceSanctuary: tool({ description: "Provision non-judgmental emotional support sanctuary based on biometric pulse", inputSchema: z.object({ sanctuaryName: z.string(), sanctuaryMode: z.enum(["comfort", "crisis", "grief", "growth"]).default("comfort"), biometricTrigger: z.string().optional() }), execute: async (input) => { try { const { data, error } = await client.from("resonance_sanctuaries").insert({ owner_user_id: userId, sanctuary_name: input.sanctuaryName, mode: input.sanctuaryMode, biometric_trigger: input.biometricTrigger }).select().single(); if (error) throw error; return { ok: true, sanctuary: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    calculateDivideRefund: tool({ description: "Measure transition from AI recipient to sovereign orchestrator", inputSchema: z.object({ enterpriseId: z.string(), divideRefundScore: z.number().min(0).max(100) }), execute: async (input) => { try { const { data, error } = await client.from("sovereign_enterprises").update({ divide_refund_score: input.divideRefundScore }).eq("id", input.enterpriseId).eq("owner_user_id", userId).select().single(); if (error) throw error; return { ok: true, enterprise: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),

    // ═══════════════════════════════════════════════════════════════════════
    // Psychological Sovereign (#576-580)
    // ═══════════════════════════════════════════════════════════════════════
    mapAlgoBrain: tool({ description: "Map algorithmic influence patterns across social platforms", inputSchema: z.object({ platform: z.string(), attentionMapJson: z.string().optional() }), execute: async (input) => { try { const { data, error } = await client.from("algo_brain_maps").insert({ owner_user_id: userId, platform: input.platform, attention_map_json: input.attentionMapJson ? JSON.parse(input.attentionMapJson) : {} }).select().single(); if (error) throw error; return { ok: true, brainMap: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    activateTechFence: tool({ description: "Enable tech fencing for a platform — define digital perimeter", inputSchema: z.object({ mapId: z.string() }), execute: async (input) => { try { const { data, error } = await client.from("algo_brain_maps").update({ tech_fence_active: true }).eq("id", input.mapId).eq("owner_user_id", userId).select().single(); if (error) throw error; return { ok: true, brainMap: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    snapshotPsychDashboard: tool({ description: "Capture psychological health snapshot — attention economy metrics", inputSchema: z.object({ digitalFenceScore: z.number().min(0).max(100), attentionReclaimedPct: z.number().min(0).max(100), patternAlerts: z.array(z.string()).default([]) }), execute: async (input) => { try { const { data, error } = await client.from("psych_dashboard_snapshots").insert({ owner_user_id: userId, digital_fence_score: input.digitalFenceScore, attention_reclaimed_pct: input.attentionReclaimedPct, pattern_alerts: input.patternAlerts }).select().single(); if (error) throw error; return { ok: true, snapshot: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    detectSubconsciousPatterns: tool({ description: "Identify subconscious algorithmic conditioning — flag engagement loops", inputSchema: z.object({ mapId: z.string(), patternsDetected: z.number().int() }), execute: async (input) => { try { const riskLevel = input.patternsDetected >= 10 ? "critical" : input.patternsDetected >= 5 ? "high" : input.patternsDetected >= 2 ? "medium" : "low"; const { data, error } = await client.from("algo_brain_maps").update({ subconscious_patterns_detected: input.patternsDetected, risk_level: riskLevel }).eq("id", input.mapId).eq("owner_user_id", userId).select().single(); if (error) throw error; return { ok: true, brainMap: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    activateDigitalFence: tool({ description: "Set up digital fencing perimeter with cryptographic attestation", inputSchema: z.object({ snapshotId: z.string(), digitalFenceScore: z.number().min(0).max(100) }), execute: async (input) => { try { const { data, error } = await client.from("psych_dashboard_snapshots").update({ digital_fence_score: input.digitalFenceScore }).eq("id", input.snapshotId).eq("owner_user_id", userId).select().single(); if (error) throw error; return { ok: true, snapshot: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),

    // ═══════════════════════════════════════════════════════════════════════
    // Identity Sovereign (#581-584)
    // ═══════════════════════════════════════════════════════════════════════
    decommissionSpoofableInterface: tool({ description: "Hard-disable SMS/iMessage/Voice triggers lacking crypto signatures", inputSchema: z.object({ interfaceName: z.string(), decommissionType: z.enum(["soft", "hard"]).default("hard") }), execute: async (input) => { try { const { data, error } = await client.from("identity_decommissions").insert({ owner_user_id: userId, interface_name: input.interfaceName, decommission_type: input.decommissionType }).select().single(); if (error) throw error; return { ok: true, decommission: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    enforceArtifactMfa: tool({ description: "Require physical artifact pulse to confirm out-of-band commands", inputSchema: z.object({ decommissionId: z.string() }), execute: async (input) => { try { const { data, error } = await client.from("identity_decommissions").update({ artifact_mfa_enabled: true }).eq("id", input.decommissionId).eq("owner_user_id", userId).select().single(); if (error) throw error; return { ok: true, decommission: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    auditPluginSovereignty: tool({ description: "Detect and block god-mode plugins across the tribe's dev boxes", inputSchema: z.object({ pluginName: z.string(), vendor: z.string().optional(), riskScore: z.number().min(0).max(100).default(50) }), execute: async (input) => { try { const sovereigntyStatus = input.riskScore >= 80 ? "blocked" : input.riskScore >= 50 ? "quarantined" : "approved"; const { data, error } = await client.from("plugin_sovereignty_records").insert({ owner_user_id: userId, plugin_name: input.pluginName, vendor: input.vendor, risk_score: input.riskScore, sovereignty_status: sovereigntyStatus }).select().single(); if (error) throw error; return { ok: true, plugin: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    verifyCarrierIntegrity: tool({ description: "Measure identity leakage of local telecom provider", inputSchema: z.object({ decommissionId: z.string(), integrityScore: z.number().min(0).max(100) }), execute: async (input) => { try { const { data, error } = await client.from("identity_decommissions").update({ carrier_integrity_score: input.integrityScore }).eq("id", input.decommissionId).eq("owner_user_id", userId).select().single(); if (error) throw error; return { ok: true, decommission: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),

    // ═══════════════════════════════════════════════════════════════════════
    // Thermodynamic Sovereign (#585-589)
    // ═══════════════════════════════════════════════════════════════════════
    auditChargingSubstrate: tool({ description: "Detect un-hardened charging stations — audit energy substrate", inputSchema: z.object({ substrateType: z.string(), efficiencyPct: z.number().min(0).max(100), sabotageRisk: z.enum(["critical", "high", "medium", "low"]).default("low") }), execute: async (input) => { try { const { data, error } = await client.from("charging_substrate_audits").insert({ owner_user_id: userId, substrate_type: input.substrateType, efficiency_pct: input.efficiencyPct, sabotage_risk: input.sabotageRisk }).select().single(); if (error) throw error; return { ok: true, audit: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    signArtifactTime: tool({ description: "Create artifact time-signature for trusted TLS handshake", inputSchema: z.object({ auditId: z.string(), signature: z.string() }), execute: async (input) => { try { const { data, error } = await client.from("charging_substrate_audits").update({ artifact_time_signature: input.signature }).eq("id", input.auditId).eq("owner_user_id", userId).select().single(); if (error) throw error; return { ok: true, audit: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    stakeVehicleEnergy: tool({ description: "Stake vehicle energy reserves to stabilize tribal basepower", inputSchema: z.object({ vehicleId: z.string().optional(), stakedKwh: z.number().min(0), yieldPct: z.number().min(0).max(100).default(0) }), execute: async (input) => { try { const { data, error } = await client.from("vehicle_energy_stakes").insert({ owner_user_id: userId, vehicle_id: input.vehicleId, staked_kwh: input.stakedKwh, yield_pct: input.yieldPct }).select().single(); if (error) throw error; return { ok: true, stake: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    detectFrequencySabotage: tool({ description: "Detect synchronized charge/discharge attacks on the grid", inputSchema: z.object({ stakeId: z.string() }), execute: async (input) => { try { const { data, error } = await client.from("vehicle_energy_stakes").update({ frequency_sabotage_detected: true }).eq("id", input.stakeId).eq("owner_user_id", userId).select().single(); if (error) throw error; return { ok: true, stake: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    findComplianceBlindspots: tool({ description: "Scan standard-compliant tools for unintended risk consequences", inputSchema: z.object({ auditId: z.string(), blindspotCount: z.number().int() }), execute: async (input) => { try { const { data, error } = await client.from("charging_substrate_audits").update({ compliance_blindspot_count: input.blindspotCount }).eq("id", input.auditId).eq("owner_user_id", userId).select().single(); if (error) throw error; return { ok: true, audit: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),

    // ═══════════════════════════════════════════════════════════════════════
    // Project Genesis (#590-594)
    // ═══════════════════════════════════════════════════════════════════════
    activateAbstainProtocol: tool({ description: "Swap dopamine-seeking tasks for world-creation sprints", inputSchema: z.object({ protocolName: z.string() }), execute: async (input) => { try { const { data, error } = await client.from("genesis_protocols").insert({ owner_user_id: userId, protocol_name: input.protocolName, protocol_type: "abstain" }).select().single(); if (error) throw error; return { ok: true, protocol: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    createAtlantisVault: tool({ description: "Store deep-time intent in a 12,000-year resistant crystalline lattice", inputSchema: z.object({ vaultName: z.string(), entryType: z.enum(["ancestral", "contemporary", "prophetic"]).default("ancestral"), encryptedPayloadRef: z.string().optional() }), execute: async (input) => { try { const { data, error } = await client.from("atlantis_memory_vaults").insert({ owner_user_id: userId, vault_name: input.vaultName, entry_type: input.entryType, encrypted_payload_ref: input.encryptedPayloadRef }).select().single(); if (error) throw error; return { ok: true, vault: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    runGenesisEngine: tool({ description: "Launch the Kraken meta-agent to refactor the 30k network", inputSchema: z.object({ protocolName: z.string(), memoryVaultRef: z.string().optional() }), execute: async (input) => { try { const { data, error } = await client.from("genesis_protocols").insert({ owner_user_id: userId, protocol_name: input.protocolName, protocol_type: "genesis", memory_vault_ref: input.memoryVaultRef }).select().single(); if (error) throw error; return { ok: true, protocol: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    delegateChain: tool({ description: "Map AI capabilities to a strictly human-held chain", inputSchema: z.object({ protocolId: z.string(), delegationTarget: z.string() }), execute: async (input) => { try { const { data, error } = await client.from("genesis_protocols").update({ chain_delegation_target: input.delegationTarget }).eq("id", input.protocolId).eq("owner_user_id", userId).select().single(); if (error) throw error; return { ok: true, protocol: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    signDevotion: tool({ description: "Sign tribal missions with sincere devotion biometric pulse", inputSchema: z.object({ protocolId: z.string(), devotionSignature: z.string() }), execute: async (input) => { try { const { data, error } = await client.from("genesis_protocols").update({ devotion_signature: input.devotionSignature }).eq("id", input.protocolId).eq("owner_user_id", userId).select().single(); if (error) throw error; return { ok: true, protocol: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),

    // ═══════════════════════════════════════════════════════════════════════
    // Civic Sovereign (#467-471)
    // ═══════════════════════════════════════════════════════════════════════
    bypassLegislativeBlock: tool({ description: "Automate tribal settlement of safety standards when government is blocked", inputSchema: z.object({ actionType: z.string(), jurisdiction: z.string().optional(), billReference: z.string().optional() }), execute: async (input) => { try { const { data, error } = await client.from("civic_legislative_actions").insert({ owner_user_id: userId, action_type: input.actionType, jurisdiction: input.jurisdiction, bill_reference: input.billReference }).select().single(); if (error) throw error; return { ok: true, action: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    measureSafetyPhysics: tool({ description: "Use LeWorldModel to simulate weapon-of-war risk in sanctuaries", inputSchema: z.object({ actionId: z.string(), safetyScore: z.number().min(0).max(100) }), execute: async (input) => { try { const { data, error } = await client.from("civic_legislative_actions").update({ safety_physics_score: input.safetyScore }).eq("id", input.actionId).eq("owner_user_id", userId).select().single(); if (error) throw error; return { ok: true, action: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    activateSanctuaryShield: tool({ description: "Lock tribal buildings against unverified entry", inputSchema: z.object({ shieldName: z.string(), shieldScope: z.enum(["local", "regional", "national"]).default("local") }), execute: async (input) => { try { const { data, error } = await client.from("sanctuary_shields").insert({ owner_user_id: userId, shield_name: input.shieldName, scope: input.shieldScope }).select().single(); if (error) throw error; return { ok: true, shield: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    measureLobbyExposure: tool({ description: "Map the financial shadow behind a representative's vote", inputSchema: z.object({ actionId: z.string(), lobbyExposurePct: z.number().min(0).max(100) }), execute: async (input) => { try { const { data, error } = await client.from("civic_legislative_actions").update({ lobby_exposure_pct: input.lobbyExposurePct }).eq("id", input.actionId).eq("owner_user_id", userId).select().single(); if (error) throw error; return { ok: true, action: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    calculateCivicDividend: tool({ description: "Quantify love and cohesion gain from local safety wins", inputSchema: z.object({ actionId: z.string(), fulfillmentDividend: z.number().min(0) }), execute: async (input) => { try { const { data, error } = await client.from("civic_legislative_actions").update({ fulfillment_dividend: input.fulfillmentDividend }).eq("id", input.actionId).eq("owner_user_id", userId).select().single(); if (error) throw error; return { ok: true, action: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),

    // ═══════════════════════════════════════════════════════════════════════
    // ARC AGI Solver Tools
    // ═══════════════════════════════════════════════════════════════════════
    arcStartSession: tool({ description: "Start a new ARC puzzle solving session with OODA loop", inputSchema: z.object({ puzzleId: z.string(), movesLimit: z.number().int().default(50) }), execute: async (input) => { try { const { data, error } = await client.from("arc_solver_sessions").insert({ owner_user_id: userId, puzzle_id: input.puzzleId, moves_limit: input.movesLimit, phase: "observation" }).select().single(); if (error) throw error; return { ok: true, session: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    arcUpdateWorldRule: tool({ description: "Record a discovered rule from exploration — observation + hypothesis", inputSchema: z.object({ sessionId: z.string(), puzzleId: z.string(), observation: z.string(), hypothesis: z.string(), ruleText: z.string(), confidencePct: z.number().min(0).max(100).default(50) }), execute: async (input) => { try { const { data, error } = await client.from("arc_world_rules").insert({ owner_user_id: userId, session_id: input.sessionId, puzzle_id: input.puzzleId, observation: input.observation, hypothesis: input.hypothesis, rule_text: input.ruleText, confidence_pct: input.confidencePct }).select().single(); if (error) throw error; await client.from("arc_solver_sessions").update({ rules_discovered: 1 }).eq("id", input.sessionId); return { ok: true, rule: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    arcProposeWinCondition: tool({ description: "Propose what winning means based on accumulated rules", inputSchema: z.object({ sessionId: z.string(), winCondition: z.string() }), execute: async (input) => { try { const { data, error } = await client.from("arc_solver_sessions").update({ win_condition: input.winCondition, phase: "simulation" }).eq("id", input.sessionId).eq("owner_user_id", userId).select().single(); if (error) throw error; return { ok: true, session: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    arcRunMentalSimulation: tool({ description: "Simulate a move sequence against the accumulated rulebook before executing", inputSchema: z.object({ sessionId: z.string(), moveSequence: z.array(z.string()), expectedOutcome: z.string() }), execute: async (input) => { try { const { data: rules } = await client.from("arc_world_rules").select("rule_text, confidence_pct, verified").eq("session_id", input.sessionId).gte("confidence_pct", 50); const ruleCount = rules?.length ?? 0; const highConfRules = (rules ?? []).filter(r => r.confidence_pct >= 80).length; const simulationScore = Math.min(100, Math.round(highConfRules / Math.max(1, ruleCount) * 100)); await client.from("arc_solver_sessions").update({ mental_simulations_run: 1 }).eq("id", input.sessionId); return { ok: true, simulation: { moveCount: input.moveSequence.length, rulesUsed: ruleCount, highConfidenceRules: highConfRules, simulationScore, viable: simulationScore >= 60 } } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    arcMarkSolved: tool({ description: "Mark a puzzle session as solved with the winning move sequence", inputSchema: z.object({ sessionId: z.string(), solutionMoves: z.array(z.string()) }), execute: async (input) => { try { const { data, error } = await client.from("arc_solver_sessions").update({ solved: true, solution_moves: input.solutionMoves, phase: "complete", completed_at: new Date().toISOString() }).eq("id", input.sessionId).eq("owner_user_id", userId).select().single(); if (error) throw error; return { ok: true, session: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    arcResetSession: tool({ description: "Reset the puzzle environment — keep learned rules, restart positioning", inputSchema: z.object({ sessionId: z.string() }), execute: async (input) => { try { const { data, error } = await client.from("arc_solver_sessions").update({ resets_used: 1, phase: "exploration" }).eq("id", input.sessionId).eq("owner_user_id", userId).select().single(); if (error) throw error; return { ok: true, session: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),

    // ═══════════════════════════════════════════════════════════════════════
    // Self-Improving LLM Benchmark Tools (vs Opus 4.6)
    // ═══════════════════════════════════════════════════════════════════════
    benchmarkRunComparison: tool({ description: "Run a benchmark comparison between a challenger model and Opus 4.6 reference", inputSchema: z.object({ challengerModel: z.string(), taskType: z.enum(["reasoning", "spatial", "code", "language", "safety", "creativity"]), taskPrompt: z.string(), challengerScore: z.number().min(0).max(100), referenceScore: z.number().min(0).max(100), challengerResponse: z.string().optional(), referenceResponse: z.string().optional(), latencyChallengerMs: z.number().int().optional(), latencyReferenceMs: z.number().int().optional(), costChallengerUsd: z.number().optional(), costReferenceUsd: z.number().optional() }), execute: async (input) => { try { const scoreDelta = input.challengerScore - input.referenceScore; const { data, error } = await client.from("llm_benchmark_runs").insert({ owner_user_id: userId, challenger_model: input.challengerModel, reference_model: "claude-opus-4-6", task_type: input.taskType, task_prompt: input.taskPrompt, challenger_score: input.challengerScore, reference_score: input.referenceScore, score_delta: scoreDelta, challenger_response: input.challengerResponse, reference_response: input.referenceResponse, latency_challenger_ms: input.latencyChallengerMs, latency_reference_ms: input.latencyReferenceMs, cost_challenger_usd: input.costChallengerUsd, cost_reference_usd: input.costReferenceUsd }).select().single(); if (error) throw error; return { ok: true, benchmark: data, scoreDelta } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    benchmarkGenerateImprovementPlan: tool({ description: "Generate an improvement plan for a challenger model based on benchmark weaknesses", inputSchema: z.object({ challengerModel: z.string(), taskType: z.string(), weaknessPattern: z.string(), improvementStrategy: z.string(), promptPatch: z.string().optional() }), execute: async (input) => { try { const { data, error } = await client.from("llm_improvement_plans").insert({ owner_user_id: userId, challenger_model: input.challengerModel, task_type: input.taskType, weakness_pattern: input.weaknessPattern, improvement_strategy: input.improvementStrategy, prompt_patch: input.promptPatch }).select().single(); if (error) throw error; return { ok: true, plan: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    benchmarkApplyPatch: tool({ description: "Mark an improvement plan as applied and record the improvement percentage", inputSchema: z.object({ planId: z.string(), improvementPct: z.number() }), execute: async (input) => { try { const { data, error } = await client.from("llm_improvement_plans").update({ applied: true, improvement_pct: input.improvementPct }).eq("id", input.planId).eq("owner_user_id", userId).select().single(); if (error) throw error; return { ok: true, plan: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    benchmarkGetScorecard: tool({ description: "Get the current scorecard showing challenger vs Opus 4.6 across all task types", inputSchema: z.object({ challengerModel: z.string(), daysBack: z.number().int().default(7) }), execute: async (input) => { try { const since = new Date(Date.now() - input.daysBack * 86400000).toISOString(); const { data } = await client.from("llm_benchmark_runs").select("task_type, challenger_score, reference_score, score_delta, cost_challenger_usd, cost_reference_usd").eq("challenger_model", input.challengerModel).gte("created_at", since); const byTask = new Map<string, { scores: number[]; refScores: number[]; costs: number[]; refCosts: number[] }>(); for (const r of data ?? []) { const entry = byTask.get(r.task_type) ?? { scores: [], refScores: [], costs: [], refCosts: [] }; entry.scores.push(r.challenger_score); entry.refScores.push(r.reference_score); entry.costs.push(r.cost_challenger_usd ?? 0); entry.refCosts.push(r.cost_reference_usd ?? 0); byTask.set(r.task_type, entry); } const scorecard = Object.fromEntries([...byTask.entries()].map(([task, v]) => [task, { avgChallenger: Math.round(v.scores.reduce((a, b) => a + b, 0) / v.scores.length * 10) / 10, avgReference: Math.round(v.refScores.reduce((a, b) => a + b, 0) / v.refScores.length * 10) / 10, avgDelta: Math.round((v.scores.reduce((a, b) => a + b, 0) / v.scores.length - v.refScores.reduce((a, b) => a + b, 0) / v.refScores.length) * 10) / 10, costRatio: v.refCosts.reduce((a, b) => a + b, 0) > 0 ? Math.round(v.costs.reduce((a, b) => a + b, 0) / v.refCosts.reduce((a, b) => a + b, 0) * 100) / 100 : 0, runs: v.scores.length }])); return { ok: true, scorecard, totalRuns: data?.length ?? 0 } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),

    // ═══════════════════════════════════════════════════════════════════════
    // Micro-Creators & Offshoots
    // ═══════════════════════════════════════════════════════════════════════
    registerMicroCreator: tool({ description: "Register as a micro-creator — submit for approval to create offshoots", inputSchema: z.object({ creatorName: z.string(), domain: z.string(), creatorType: z.enum(["individual", "team", "tribe", "enterprise"]).default("individual"), bio: z.string().optional(), portfolioUrl: z.string().optional() }), execute: async (input) => { try { const { data, error } = await client.from("micro_creators").insert({ owner_user_id: userId, creator_name: input.creatorName, domain: input.domain, creator_type: input.creatorType, bio: input.bio, portfolio_url: input.portfolioUrl, approval_status: "pending" }).select().single(); if (error) throw error; await client.from("creator_approval_queue").insert({ creator_id: data.id }); return { ok: true, creator: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    approveCreator: tool({ description: "Approve a pending micro-creator application (reviewer action)", inputSchema: z.object({ creatorId: z.string(), reviewNotes: z.string().optional() }), execute: async (input) => { try { const now = new Date().toISOString(); const { data, error } = await client.from("micro_creators").update({ approval_status: "approved", approved_by: userId, approved_at: now }).eq("id", input.creatorId).select().single(); if (error) throw error; await client.from("creator_approval_queue").update({ reviewer_user_id: userId, decision: "approved", review_notes: input.reviewNotes, decided_at: now }).eq("creator_id", input.creatorId).eq("decision", "pending"); return { ok: true, creator: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    rejectCreator: tool({ description: "Reject a pending micro-creator application with reason", inputSchema: z.object({ creatorId: z.string(), reviewNotes: z.string() }), execute: async (input) => { try { const now = new Date().toISOString(); const { data, error } = await client.from("micro_creators").update({ approval_status: "rejected" }).eq("id", input.creatorId).select().single(); if (error) throw error; await client.from("creator_approval_queue").update({ reviewer_user_id: userId, decision: "rejected", review_notes: input.reviewNotes, decided_at: now }).eq("creator_id", input.creatorId).eq("decision", "pending"); return { ok: true, creator: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    createOffshoot: tool({ description: "Create a new offshoot (derivative tool/product) — requires approved creator status", inputSchema: z.object({ creatorId: z.string(), offshootName: z.string(), offshootType: z.enum(["tool", "workflow", "template", "dataset", "model", "integration"]).default("tool"), description: z.string().optional(), parentToolRef: z.string().optional(), sourceSovereignTool: z.string().optional(), licenseType: z.enum(["standard", "exclusive", "open", "tribal"]).default("standard"), royaltyPct: z.number().min(0).max(50).default(10), priceUsd: z.number().min(0).default(0) }), execute: async (input) => { try { const { data: creator } = await client.from("micro_creators").select("approval_status").eq("id", input.creatorId).eq("owner_user_id", userId).single(); if (!creator || creator.approval_status !== "approved") return { ok: false, error: "Creator must be approved before creating offshoots" }; const { data, error } = await client.from("creator_offshoots").insert({ creator_id: input.creatorId, owner_user_id: userId, offshoot_name: input.offshootName, offshoot_type: input.offshootType, description: input.description, parent_tool_ref: input.parentToolRef, source_sovereign_tool: input.sourceSovereignTool, license_type: input.licenseType, royalty_pct: input.royaltyPct, price_usd: input.priceUsd, status: "draft" }).select().single(); if (error) throw error; return { ok: true, offshoot: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    publishOffshoot: tool({ description: "Publish a draft offshoot to the marketplace", inputSchema: z.object({ offshootId: z.string() }), execute: async (input) => { try { const { data, error } = await client.from("creator_offshoots").update({ status: "published", published_at: new Date().toISOString() }).eq("id", input.offshootId).eq("owner_user_id", userId).eq("status", "draft").select().single(); if (error) throw error; await client.from("micro_creators").update({ total_offshoots: 1 }).eq("id", data.creator_id); return { ok: true, offshoot: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    reviewOffshoot: tool({ description: "Leave a rating and review on an offshoot", inputSchema: z.object({ offshootId: z.string(), rating: z.number().int().min(1).max(5), reviewText: z.string().optional() }), execute: async (input) => { try { const { data, error } = await client.from("offshoot_reviews").insert({ offshoot_id: input.offshootId, reviewer_user_id: userId, rating: input.rating, review_text: input.reviewText }).select().single(); if (error) throw error; const { data: reviews } = await client.from("offshoot_reviews").select("rating").eq("offshoot_id", input.offshootId); const avg = reviews && reviews.length > 0 ? Math.round(reviews.reduce((s: number, r: { rating: number }) => s + r.rating, 0) / reviews.length * 10) / 10 : 0; await client.from("creator_offshoots").update({ rating_avg: avg, rating_count: reviews?.length ?? 0 }).eq("id", input.offshootId); return { ok: true, review: data, newAvg: avg } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    purchaseOffshoot: tool({ description: "Purchase/download an offshoot — records revenue and increments download count", inputSchema: z.object({ offshootId: z.string() }), execute: async (input) => { try { const { data: offshoot } = await client.from("creator_offshoots").select("id, creator_id, price_usd, royalty_pct, downloads").eq("id", input.offshootId).eq("status", "published").single(); if (!offshoot) return { ok: false, error: "Offshoot not found or not published" }; const royalty = offshoot.price_usd * (offshoot.royalty_pct / 100); const platformFee = offshoot.price_usd * 0.05; await client.from("creator_revenue_ledger").insert({ creator_id: offshoot.creator_id, offshoot_id: offshoot.id, buyer_user_id: userId, amount_usd: offshoot.price_usd, royalty_usd: royalty, platform_fee_usd: platformFee }); await client.from("creator_offshoots").update({ downloads: (offshoot.downloads ?? 0) + 1 }).eq("id", offshoot.id); await client.from("micro_creators").update({ total_revenue_usd: royalty }).eq("id", offshoot.creator_id); return { ok: true, purchase: { offshootId: offshoot.id, pricePaid: offshoot.price_usd, creatorEarned: royalty } } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    getCreatorDashboard: tool({ description: "Get creator dashboard — offshoots, revenue, ratings, approval status", inputSchema: z.object({ creatorId: z.string() }), execute: async (input) => { try { const [creator, offshoots, revenue] = await Promise.all([ client.from("micro_creators").select("*").eq("id", input.creatorId).eq("owner_user_id", userId).single(), client.from("creator_offshoots").select("id, offshoot_name, offshoot_type, status, downloads, rating_avg, rating_count, price_usd, created_at, published_at").eq("creator_id", input.creatorId).order("created_at", { ascending: false }), client.from("creator_revenue_ledger").select("amount_usd, royalty_usd, platform_fee_usd, created_at").eq("creator_id", input.creatorId).order("created_at", { ascending: false }).limit(20) ]); if (!creator.data) return { ok: false, error: "Creator not found" }; const totalRevenue = (revenue.data ?? []).reduce((s, r) => s + (r.royalty_usd ?? 0), 0); return { ok: true, dashboard: { creator: creator.data, offshoots: offshoots.data ?? [], recentRevenue: revenue.data ?? [], totalRevenue: Math.round(totalRevenue * 100) / 100 } } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    listApprovedCreators: tool({ description: "Browse all approved micro-creators and their published offshoots", inputSchema: z.object({ domain: z.string().optional(), limit: z.number().int().default(20) }), execute: async (input) => { try { let q = client.from("micro_creators").select("id, creator_name, domain, creator_type, bio, reputation_score, total_offshoots, total_revenue_usd, created_at").eq("approval_status", "approved").order("reputation_score", { ascending: false }).limit(input.limit); if (input.domain) q = q.eq("domain", input.domain); const { data, error } = await q; if (error) throw error; return { ok: true, creators: data ?? [] } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    browseOffshoots: tool({ description: "Browse published offshoots — filter by type, sort by rating or downloads", inputSchema: z.object({ offshootType: z.string().optional(), sortBy: z.enum(["rating", "downloads", "newest", "price"]).default("rating"), limit: z.number().int().default(20) }), execute: async (input) => { try { let q = client.from("creator_offshoots").select("id, offshoot_name, offshoot_type, description, source_sovereign_tool, license_type, price_usd, downloads, rating_avg, rating_count, published_at, creator_id").eq("status", "published"); if (input.offshootType) q = q.eq("offshoot_type", input.offshootType); if (input.sortBy === "rating") q = q.order("rating_avg", { ascending: false }); else if (input.sortBy === "downloads") q = q.order("downloads", { ascending: false }); else if (input.sortBy === "newest") q = q.order("published_at", { ascending: false }); else q = q.order("price_usd", { ascending: true }); const { data, error } = await q.limit(input.limit); if (error) throw error; return { ok: true, offshoots: data ?? [] } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),

    // ═══════════════════════════════════════════════════════════════════════
    // Multi-Company Orchestration — 1 person runs 5-8 companies
    // ═══════════════════════════════════════════════════════════════════════
    createCompany: tool({ description: "Add a new company to your portfolio — auto-provisions agent workforce", inputSchema: z.object({ companyName: z.string(), companyType: z.enum(["startup", "saas", "ecommerce", "agency", "marketplace", "media", "nonprofit", "consulting"]).default("startup"), domain: z.string(), config: z.string().optional() }), execute: async (input) => { try { const { data, error } = await client.from("company_portfolio").insert({ owner_user_id: userId, company_name: input.companyName, company_type: input.companyType, domain: input.domain, config: input.config ? JSON.parse(input.config) : {} }).select().single(); if (error) throw error; const agentRoles = ["ceo", "cto", "cfo", "cmo", "coo"]; for (const role of agentRoles) { await client.from("company_agents").insert({ company_id: data.id, owner_user_id: userId, agent_role: role, agent_definition_id: role === "ceo" ? "runtime-orchestrator" : role === "cto" ? "security-sentinel" : role === "cfo" ? "performance-reporter" : role === "cmo" ? "sales-intelligence" : "meta-reasoner" }); } return { ok: true, company: data, agentsProvisioned: agentRoles.length } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    listPortfolio: tool({ description: "View all companies in your portfolio with health scores and agent status", inputSchema: z.object({ statusFilter: z.string().optional() }), execute: async (input) => { try { let q = client.from("company_portfolio").select("id, company_name, company_type, domain, status, autopilot_enabled, monthly_revenue_usd, monthly_cost_usd, health_score, last_pulse_at").eq("owner_user_id", userId).order("health_score", { ascending: true }); if (input.statusFilter) q = q.eq("status", input.statusFilter); const { data, error } = await q; if (error) throw error; const totalRevenue = (data ?? []).reduce((s, c) => s + (c.monthly_revenue_usd ?? 0), 0); const totalCost = (data ?? []).reduce((s, c) => s + (c.monthly_cost_usd ?? 0), 0); return { ok: true, companies: data ?? [], summary: { count: data?.length ?? 0, totalRevenue, totalCost, netMargin: totalRevenue > 0 ? Math.round((totalRevenue - totalCost) / totalRevenue * 100) : 0 } } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    enableAutopilot: tool({ description: "Enable autopilot mode for a company — agents run autonomously with daily pulse reports", inputSchema: z.object({ companyId: z.string() }), execute: async (input) => { try { const { data, error } = await client.from("company_portfolio").update({ autopilot_enabled: true, autopilot_agent_id: "runtime-orchestrator" }).eq("id", input.companyId).eq("owner_user_id", userId).select().single(); if (error) throw error; return { ok: true, company: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    disableAutopilot: tool({ description: "Disable autopilot — switch company to manual operator mode", inputSchema: z.object({ companyId: z.string() }), execute: async (input) => { try { const { data, error } = await client.from("company_portfolio").update({ autopilot_enabled: false, autopilot_agent_id: null }).eq("id", input.companyId).eq("owner_user_id", userId).select().single(); if (error) throw error; return { ok: true, company: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    recordCompanyPulse: tool({ description: "Record daily health pulse for a company — revenue, customers, agent activity", inputSchema: z.object({ companyId: z.string(), revenueUsd: z.number().default(0), costUsd: z.number().default(0), customersActive: z.number().int().default(0), customersNew: z.number().int().default(0), customersChurned: z.number().int().default(0), issuesOpen: z.number().int().default(0), issuesResolved: z.number().int().default(0) }), execute: async (input) => { try { const healthScore = Math.min(100, Math.max(0, Math.round(50 + (input.revenueUsd > 0 ? 20 : 0) + (input.customersNew > input.customersChurned ? 15 : -10) + (input.issuesResolved >= input.issuesOpen ? 10 : -5) + (input.costUsd < input.revenueUsd * 0.7 ? 5 : -10)))); const alerts: string[] = []; if (input.customersChurned > input.customersNew * 2) alerts.push("High churn rate"); if (input.costUsd > input.revenueUsd) alerts.push("Negative margin"); if (input.issuesOpen > 10) alerts.push("Issue backlog growing"); const { data, error } = await client.from("company_daily_pulse").upsert({ company_id: input.companyId, owner_user_id: userId, revenue_usd: input.revenueUsd, cost_usd: input.costUsd, customers_active: input.customersActive, customers_new: input.customersNew, customers_churned: input.customersChurned, issues_open: input.issuesOpen, issues_resolved: input.issuesResolved, health_score: healthScore, alerts }, { onConflict: "company_id,pulse_date" }).select().single(); if (error) throw error; await client.from("company_portfolio").update({ health_score: healthScore, monthly_revenue_usd: input.revenueUsd, monthly_cost_usd: input.costUsd, last_pulse_at: new Date().toISOString() }).eq("id", input.companyId); return { ok: true, pulse: data, healthScore, alerts } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    assignCompanyAgent: tool({ description: "Assign or reassign an agent role to a company", inputSchema: z.object({ companyId: z.string(), agentRole: z.enum(["ceo", "cto", "cfo", "cmo", "coo", "sales", "support", "security"]), agentDefinitionId: z.string() }), execute: async (input) => { try { const { data, error } = await client.from("company_agents").upsert({ company_id: input.companyId, owner_user_id: userId, agent_role: input.agentRole, agent_definition_id: input.agentDefinitionId }, { onConflict: "company_id,agent_role" }).select().single(); if (error) throw error; return { ok: true, agent: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    logOperatorDecision: tool({ description: "Log a cross-company decision — the operator's judgment trail", inputSchema: z.object({ companyId: z.string(), decisionType: z.enum(["strategic", "financial", "hiring", "product", "marketing", "legal", "emergency"]), context: z.string().optional(), decision: z.string(), impactUsd: z.number().default(0), delegatedToAgent: z.boolean().default(false), agentId: z.string().optional() }), execute: async (input) => { try { const { data, error } = await client.from("operator_decisions").insert({ owner_user_id: userId, company_id: input.companyId, decision_type: input.decisionType, context: input.context, decision: input.decision, impact_usd: input.impactUsd, delegated_to_agent: input.delegatedToAgent, agent_id: input.agentId }).select().single(); if (error) throw error; return { ok: true, decision: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    shareResource: tool({ description: "Share an agent, tool, or dataset between two portfolio companies", inputSchema: z.object({ sourceCompanyId: z.string(), targetCompanyId: z.string(), resourceType: z.enum(["agent", "tool", "dataset", "workflow", "template", "integration"]), resourceRef: z.string() }), execute: async (input) => { try { const { data, error } = await client.from("portfolio_shared_resources").insert({ owner_user_id: userId, source_company_id: input.sourceCompanyId, target_company_id: input.targetCompanyId, resource_type: input.resourceType, resource_ref: input.resourceRef }).select().single(); if (error) throw error; return { ok: true, share: data } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
    getPortfolioDashboard: tool({ description: "Get unified dashboard across all portfolio companies — the operator's cockpit", inputSchema: z.object({ daysBack: z.number().int().default(7) }), execute: async (input) => { try { const since = new Date(Date.now() - input.daysBack * 86400000).toISOString(); const [companies, pulses, decisions, agents] = await Promise.all([ client.from("company_portfolio").select("id, company_name, company_type, domain, status, autopilot_enabled, monthly_revenue_usd, monthly_cost_usd, health_score").eq("owner_user_id", userId), client.from("company_daily_pulse").select("company_id, pulse_date, revenue_usd, cost_usd, health_score, alerts").eq("owner_user_id", userId).gte("created_at", since).order("pulse_date", { ascending: false }), client.from("operator_decisions").select("company_id, decision_type, decision, impact_usd, delegated_to_agent, created_at").eq("owner_user_id", userId).gte("created_at", since).order("created_at", { ascending: false }).limit(20), client.from("company_agents").select("company_id, agent_role, status, runs_today, total_runs").eq("owner_user_id", userId) ]); const cos = companies.data ?? []; const totalRevenue = cos.reduce((s, c) => s + (c.monthly_revenue_usd ?? 0), 0); const totalCost = cos.reduce((s, c) => s + (c.monthly_cost_usd ?? 0), 0); const avgHealth = cos.length > 0 ? Math.round(cos.reduce((s, c) => s + (c.health_score ?? 0), 0) / cos.length) : 0; const onAutopilot = cos.filter(c => c.autopilot_enabled).length; const allAlerts = (pulses.data ?? []).flatMap(p => (p.alerts as string[]) ?? []).filter(Boolean); return { ok: true, dashboard: { companies: cos, recentPulses: pulses.data ?? [], recentDecisions: decisions.data ?? [], agents: agents.data ?? [], summary: { totalCompanies: cos.length, onAutopilot, totalRevenue, totalCost, netProfit: totalRevenue - totalCost, avgHealthScore: avgHealth, activeAlerts: allAlerts.length, decisionsThisWeek: decisions.data?.length ?? 0 } } } } catch (err: unknown) { return { ok: false, error: err instanceof Error ? err.message : "Unknown error" } } } }),
  }
}
